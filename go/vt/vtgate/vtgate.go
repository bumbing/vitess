...
func Init(ctx context.Context, hc discovery.HealthCheck, serv srvtopo.Server, cell string, retryCount int, tabletTypesToWait []topodatapb.TabletType) *VTGate {
	...
	// Pinterest-specific logic to help with keeping stats on the scatter cache vindex cache size.
	...
	_ = stats.NewRates("QPSByOperation", stats.CounterForDimension(rpcVTGate.timings, "Operation"), 15*60/5, 5*time.Second)
