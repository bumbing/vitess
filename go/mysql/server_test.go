...
// TestTLSServer_Pinterest tests that TLS is required for certain roles
// with restrictions on what the certificate common name must contain.
func TestTLSServer_Pinterest(t *testing.T) {
	th := &testHandler{}
	...
	testCases := []struct {
		...
		// no TLS, no required TLS for that user
		{
			...
			useTLS: false,
			groupTLSRegexes: map[string]*regexp.Regexp{
				"group_requiring_tls": regexp.MustCompile("must_be_in_cn"),
			},
			wantErr: "",
		},
		// user with no TLS but requires TLS
		{
			...
			wantErr: `unknown error: checkUserAllowed: username:"user1" groups:"group_requiring_tls"  requires a TLS connection (errno 1105) (sqlstate HY000)`,
		},
		// TLS but no valid CN
		{
			user:   "user1",
			...
			wantErr: `unknown error: checkUserAllowed: username:"user1" groups:"group_requiring_tls"  requires a TLS CN matching must_be_in_cn but only found []string{"Client Cert"} (errno 1105) (sqlstate HY000)`,
		},
		// TLS with valid CN
		{
			user:   "user1",
			...
			useTLS: true,
			groupTLSRegexes: map[string]*regexp.Regexp{
				"group_requiring_tls": regexp.MustCompile("^Client Cert$"),
			},
			wantErr: "",
		},
	}
	...
	for _, testCase := range testCases {
		l.groupTLSRegexes = testCase.groupTLSRegexes
		...
		if err != nil {
			if testCase.wantErr == "" {
				t.Fatalf("unexpected mysql connect failure: %v. Test case: %v", err, testCase)
			} else if testCase.wantErr != err.Error() {
				t.Fatalf("wrong err. Want: %v, Got %v. Test case: %v", testCase.wantErr, err, testCase)
			}
		} else if testCase.wantErr != "" {
			t.Fatalf("No connect error. Want: %v. Test case: %v", testCase.wantErr, testCase)
		}
	}
}
