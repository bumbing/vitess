...
docker run -h=`hostname -f` -t -v `pwd`:/vt/src -v $OUTPUT_LOCAL_DIR:/vt/build_artifacts -w /vt/src --entrypoint /bin/bash 998131032990.dkr.ecr.us-east-1.amazonaws.com/bazel:latest -c "./bazel_bootstrap.sh && bazel build --workspace_status_command=./scripts/workspace_status.sh --symlink_prefix=/ :full_dist && cp -f \`bazel info bazel-bin\`/full_dist.tar.gz /vt/build_artifacts/$OUTPUT_TARBALL_NAME"
